            ...
            bindEvents: function () {
                ...
                $('#cwd-grid td.cwd-tile-active').click(function () {
                    ...
                    if (acrossClueId && prevDirection === '') {
                        clueId = acrossClueId;
                        var div_terms = $("#cwd-clueText-" + clueId).offset();
                        impl.scrollToDivAcross(div_terms);
                    } else {
                        if (downClueId && prevDirection !== 'd') {
                            clueId = downClueId;
                            var div_terms = $("#cwd-clueText-" + clueId).offset();

                            impl.scrollToDivDown(div_terms);
                        }
                    }
                    impl.showClue(clueId);


                    impl.relatedClues(clueId);
                });
                ...
                $(document).keydown(function (event) {
                    ...
                    if (highlightedTiles.length > 0) {
                        ...
                        if (code === 8) {
                            ...
                            if (cursorIndex < highlightedTiles.length) {
                            ...
                            } else if (cursorIndex == highlightedTiles.length) {
                                ...
                                $('#cwd-clue-box-text').fadeOut('fast');
                                 ...
                                 var curclue=impl.findClueById(clueId);
                                 
                                 
                                 console.log(Number(curclue.index+1));
                                 var nextclue=impl.findClueById(Number(curclue.index+1));
                                  impl.highlightTiles(nextclue.id);
...
//                              highlightedTiles.filter(impl.stringFormat('[cursorIndex={0}]', cursorIndex)).addClass('cwd-tile-cursor');
            ...
            findClueByIndex: function (index) {
//                return (index < clues.length) ? clues[index] : null;
            ...
            appendClues: function (data, onRight) {
                ...
                var acrossClues = impl.findCluesByDirection('a');
                addClues(acrossClues, 'across');
                // Down clues.

                var downClues = impl.findCluesByDirection('d');
                addClues(downClues, 'down');
...
//                  console.log(clueId);
            ...
            scrollToDivAcross: function (target) {

                $('.across-clue').animate({scrollTop: target.top - 600}, "slow");


            },
            scrollToDivDown: function (target) {

                $('.down-clue').animate({scrollTop: target.top - 600}, "slow");


            },
            relatedClues: function (clueId) {
                var clue = impl.findClueById(clueId);
                var html = '<div class="cwd-clues-header">Related Clues </div>';
                $.each(clue.related, function (ind, ele) {
                    var dir = '';
                    var clueArray = impl.findClueByIndex(ele);
                    if (clueArray[0].direction === 'a') {
                        // clueHeader('ACROSS').appendTo(cluesDiv);
                        dir = 'A';
                    } else if (clueArray[0].direction === 'd') {
                        dir = 'D';
                    }

                    html += impl.stringFormat(
                            '<div class="cwd-clue"><div class="cwd-clue-no"><b>{0}{1}</b></div><div id="cwd-clueText-{2}" class="cwd-clue-text">  {3}</div></div>',
                            clueArray[0].number,
                            dir,
                            clueArray[0].id,
                            clueArray[0].text ? clueArray[0].text : '',
                            );
                });
                $('#relatedclues').html(html);
